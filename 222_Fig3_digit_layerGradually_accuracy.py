...
nn=5;imm=200
score=np.zeros([imm,nn])
for ll in range(nn):
    clf = LogisticRegression(random_state=0, solver='lbfgs',multi_class='multinomial').fit(act1[:,start1[ll]:start1[ll+1]], train_l)
# score=clf.score(act2[:,480:512], train2_l)
    for ii in range(imm):
        ind=np.random.choice(BATCH_SIZE*2, BATCH_SIZE)
        score[ii,ll] = clf.score(act2[ind, start1[ll]:start1[ll+1]], train2_l[ind])
        # print(score[ii])
plt.boxplot(score)
plt.savefig('_Fig3/pred_accuracy_Layers.png')
np.save('pred_accuracy_chgno.npy',score)
