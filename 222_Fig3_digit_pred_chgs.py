...
def affine_transform_rand_s(b_x):
    xx = 0  # (-0.5,0.5)
    yy = 0
    sx=np.random.uniform(low=0.5, high=0.7, size=1)  # (-0.5,0.5)
    sy = np.random.uniform(low=0.5, high=0.7, size=1)# (.5,1.2)
    rr = 0  # (-0.5,0.5)
    sz1, sz2 = b_x.shape
    images = b_x.reshape([sz1, 28, 28, 1])
    seq = iaa.Sequential(
...
for i in range(BATCH_SIZE):
    ...
    train_x[i, :] = affine_transform_rand_s(np.reshape(train_x[i], [1, 784]))  # rtt
...
for i in range(BATCH_SIZE):
    ...
    train2_x[i, :]= affine_transform_rand_s(np.reshape(train2_x[i], [1, 784]))
