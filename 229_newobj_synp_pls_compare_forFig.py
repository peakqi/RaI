...
m1=np.max([np.abs(old49[:,0:10]),np.abs(new49[:,0:10])])
m0=np.max([np.abs(we0_49[:,0:10]),np.abs(we0_new[:,0:10]),np.abs(we0_old[:,0:10])])

ind=np.arange(480,512)
we0_49[0,:]=m0;we0_49[783,:]=-1*m0
we0_old[0,:]=10;we0_old[783,:]=-10
we0_new[0,:]=10;we0_new[783,:]=-10
new49[0,:]=1;new49[783,:]=-1
old49[0,:]=1;old49[783,:]=-1

mnist = input_data.read_data_sets('./mnist', one_hot=False)
b_x, b_y = mnist.train.next_batch(BATCH_SIZE)
for ii in range(10):
    b_x[ii,:], _ = affine_transform1(np.reshape(b_x[ii,:],[1,784]))
view_decoded_data, we0_digi, we1, we2, we3, wem, wd0, wd1, wd2, wd3, wdd, en0_, en1_, en2_, en3_, ff_encoded_, de0_, de1_, de2_, de3_, ddr_ = sess.run(
    [decoded, weights_en0, weights_en1, weights_en2, weights_en3, weights_mid,
     weights_de0, weights_de1, weights_de2, weights_de3, weights_ddr, en0, en1, en2, en3, ff_encoded, de0, de1,
     de2, de3, decoded],
    {tf_x: b_x,
     ...
     ph_switch: ph_switch_,
     ph_dis_e: ph_dis_e_})
act_digi = np.concatenate([en0_, en1_, en2_, en3_, ff_encoded_, de0_, de1_, de2_, de3_], axis=1);


f,a=plt.subplots(9,10)
for ii in range(10):
    a[0][ii].set_yticks(());
    a[0][ii].set_xticks(());
    a[0][ii].imshow(np.reshape(we0_49[:, ii], [28, 28]), cmap='seismic')
    a[1][ii].set_yticks(());
    a[1][ii].set_xticks(());
    a[1][ii].imshow(np.reshape(we0_old[:, ii], [28, 28]), cmap='seismic')
    a[2][ii].set_yticks(());
    a[2][ii].set_xticks(());
    a[2][ii].imshow(np.reshape(old49[:, ii], [28, 28]), cmap='seismic')

    a[3][ii].set_yticks(());
    a[3][ii].set_xticks(());
    a[3][ii].imshow(np.reshape(we0_new[:, ii], [28, 28]), cmap='seismic')
    a[4][ii].set_yticks(());
    a[4][ii].set_xticks(());
    a[4][ii].imshow(np.reshape(new49[:, ii], [28, 28]), cmap='seismic')

    a[5][ii].set_yticks(());
    a[5][ii].set_xticks(());
    a[5][ii].scatter(col_ind[ind],act_49[ii,ind],col_ind[ind]*0+0.1,col[ind])
    a[5][ii].set_ylim([0,1])

    a[6][ii].set_yticks(());
    a[6][ii].set_xticks(());
    a[6][ii].scatter(col_ind[ind],act_old[ii,ind]-act_49[ii,ind],col_ind[ind]*0+0.1,col[ind])
    a[6][ii].set_ylim([-1,1])

    a[7][ii].set_yticks(());
    a[7][ii].set_xticks(());
    a[7][ii].scatter(col_ind[ind],act_new[ii,ind]-act_49[ii,ind],col_ind[ind]*0+0.1,col[ind])
    a[7][ii].set_ylim([-1,1])

    a[8][ii].set_yticks(());
    a[8][ii].set_xticks(());
    a[8][ii].scatter(col_ind[ind],act_digi[ii,ind],col_ind[ind]*0+0.1,col[ind])
    a[8][ii].set_ylim([0,1])


plt.savefig('./W_plot/W.png')
