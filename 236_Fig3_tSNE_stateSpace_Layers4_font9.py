...
plt.savefig('./tSNE/'+'font9_tsne'+str(kk)+'.png')
...
for jj in range(4):
    ...
    for ii in range(plot_only):
        ...
        plt.text(x, y, iinn[32-jj-1], backgroundcolor=c, fontsize=9 )
    plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
    plt.savefig('./tSNE/'+'font9_tsne_importNeuron_'+str(jj)+'.png')
...
# for x, y, s ,xxx in zip(X[:plot_only], Y[:plot_only], test_l[:plot_only],para[:,0]):
    ...
    plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/'+'font9_tsne_layer'+str(kk)+'_x'+'.png')
...
plt.savefig('./tSNE/' + 'font9_tsne_layer' + str(kk) + '_y' + '.png')
...
for ii in range(BATCH_SIZE):
    ...
    plt.xlim(X.min(), X.max());
    plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/' + 'font9_tsne_layer' + str(kk) + '_s' + '.png')
...
for ii in range(BATCH_SIZE):
    ...
    plt.xlim(X.min(), X.max());
    plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/' + 'font9_tsne_layer' + str(kk) + '_r' + '.png')
    ...
    for ii in range(BATCH_SIZE):
        if test_l[ii] == jj:
            ...
            if np.sum(np.sum(np.multiply(IM,im)))==0:
                IM=IM+im
    scipy.misc.imsave('./tSNE/'+ 'font9_tsne_layer' + str(kk)+'digit'+str(jj)+'tsne.png', IM)
...
for ii in range(BATCH_SIZE):
        ...
        if np.sum(np.sum(np.multiply(IM,im)))==0:
            IM=IM+im
scipy.misc.imsave('./tSNE/'+ 'font9_tsne_layer' + str(kk)+'all_'+'font9_tsne.png', IM)
