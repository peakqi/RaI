...
for step in range(rangemax * nx, rangemax * (nx + 1)):
    ...
    if step % rangedvd == 0:
        ...
        for i in range(N_TEST_IMG):
            ...
            # action
            a[4][i].scatter(np.arange(sz_act_length), act[i], np.ones(sz_act_length) *0.01,col.reshape([sz_act_length]))
            a[4][i].set_ylim((0, 1))

            # delta_action
            a[5][i].clear()
            a[5][i].scatter(np.arange(sz_act_length), dact[i], np.ones(sz_act_length) *0.01,col.reshape([sz_act_length]))
            a[5][i].set_ylim((-1, 1))

            # averaged W
            a[6][i].scatter(np.arange(count), wd_avg[:, i], np.ones(count) *0.01, np.ones(count))

            #posi neuron
            a[7][i].scatter(np.arange(count), wd_posi_num[:, i], np.ones(count) *0.01, np.ones(count))
            a[8][i].scatter(np.arange(count), wd_posi_avg[:, i], np.ones(count) *0.01, np.ones(count))

            #neg neuron
            a[9][i].scatter(np.arange(count), wd_neg_num[:, i], np.ones(count) *0.01, np.ones(count))
            a[10][i].scatter(np.arange(count), wd_neg_avg[:, i], np.ones(count) *0.01, np.ones(count))

            # averaged abs W
            a[11][i].scatter(np.arange(count), wd_absavg[:, i], np.ones(count) *0.01, np.ones(count))
            a[11][i].set_ylim([0,0.37])
            # averaged W(t)-W(t-1)
            a[12][i].scatter(np.arange(count), wd_diff_avg[:, i], np.ones(count) *0.01, np.ones(count))

            # averaged |W(t)-W(t-1)|
            a[13][i].scatter(np.arange(count), wd_diff_absavg[:, i], np.ones(count) *0.01, np.ones(count))
        ...
        a[14][0].scatter(np.arange(count), loss_rec, np.ones(count) *0.01, np.ones(count))
