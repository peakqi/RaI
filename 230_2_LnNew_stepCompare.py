...
saver.restore(sess, '/Users/fengqi/Pycharm_py36/QF/LnNew_Forgetting/'+str(lnstep)+'/' + type)
...
test_digi=np.zeros([BATCH_SIZE,784])
...
for i in range(BATCH_SIZE):
    if jj==b_y[i]:
        print(jj)
        print(b_y[i])
        test_digi[jj, :], _ = affine_transform1(np.reshape(b_x[i, :], [1, 784]))
        a[2][jj].imshow(np.reshape(test_digi[jj, :], [28, 28]), cmap='gray')
        jj=jj+1
        if jj==10:
            break;
...
for ii in range(10):
    view_decoded_x = sess.run(decoded,{tf_x: np.reshape(test_x[ii,:],[1,784]),ph_encoded: ph_encoded_,ph_switch: ph_switch_,ph_dis_e: ph_dis_e_})
    view_decoded_digi = sess.run(decoded,{tf_x:np.reshape(test_digi[ii,:],[1,784]), ph_encoded: ph_encoded_, ph_switch: ph_switch_, ph_dis_e: ph_dis_e_})
    a[1][ii].imshow(np.reshape(view_decoded_x,[28,28]),cmap='gray')
    a[3][ii].imshow(np.reshape(view_decoded_digi,[28,28]),cmap='gray')
plt.savefig('LnNew_Forgetting/figcompare-step'+str(lnstep)+'.png')
