...
lnstep_list=[-1,0,10000,14000,20000,100000,600000,1400000,2500000,4900000]
col_sz=len(lnstep_list)

f,a=plt.subplots(col_sz,10)
for ii in range(10):
    for jj in range(col_sz):
        a[jj][ii].set_xticks(());
        a[jj][ii].set_yticks(());
im=test_x[0:10,:]
for kk in range(col_sz):
    if kk==0:
        for ii in range(10):
            a[kk][ii].imshow(np.reshape(im[ii,:], [28, 28]), cmap='gray')
    else:
        lnstep=lnstep_list[kk]
        saver.restore(sess, '/Users/fengqi/Pycharm_py36/QF/_ckpt/' + str(lnstep) + '/' + type)
        for ii in range(10):
            decoded_im = sess.run(decoded,{tf_x: np.reshape(im[ii,:],[1,784]),ph_encoded: ph_encoded_,ph_switch: ph_switch_,ph_dis_e: ph_dis_e_})
            a[kk][ii].imshow(np.reshape(decoded_im,[28,28]),cmap='gray')
plt.subplots_adjust(left=0.4,right=1,bottom=0,top=1,wspace=0,hspace=0)
plt.savefig('_Fig1/a_recon.png')
...
for kk in range(col_sz):
    if kk==0:
        ...
        we0_ = sess.run(weights_en0,
                        {tf_x: np.reshape(im[ii, :], [1, 784]), ph_encoded: ph_encoded_, ph_switch: ph_switch_,
                         ph_dis_e: ph_dis_e_})
        # aa = np.max(np.abs(we0_))
        # print(aa)
        # mm=aa

        for ii in range(10):
            W=np.reshape(we0_[:,ii], [28, 28]);
            W[0][0]=mm;W[27][27]=-1*mm
            W[W>mm]=mm;W[W<-1*mm]=-1*mm;
            a[kk][ii].imshow(W,cmap='bwr')
plt.subplots_adjust(left=0.4,right=1,bottom=0,top=1,wspace=0,hspace=0)
plt.savefig('_Fig1/a_W.png')
...
for kk in range(col_sz):
    if kk==0:
        ...
        for ii in range(10):
            ...
            a[kk][ii].imshow(W,cmap='bwr')
plt.subplots_adjust(left=0.4,right=1,bottom=0,top=1,wspace=0,hspace=0)
plt.savefig('_Fig1/a_W_PosiNeg.png')
        ...
        for ii in range(10):
            view_decoded_data, we0, we1, we2, we3, wem, wd0, wd1, wd2, wd3, wdd, en0_, en1_, en2_, en3_, ff_encoded_, de0_, de1_, de2_, de3_, ddr_ = sess.run(
                [decoded, weights_en0, weights_en1, weights_en2, weights_en3, weights_mid,
                 weights_de0, weights_de1, weights_de2, weights_de3, weights_ddr, en0, en1, en2, en3, ff_encoded, de0,
                 de1,
                 de2, de3, decoded],
                {tf_x: np.reshape(im[ii, :], [1, 784]),
            ...
            a[kk][ii].scatter(np.arange(sz_act_length), act, np.ones(sz_act_length) * 0.1,
                            col.reshape([sz_act_length]),cmap='prism')
            a[kk][ii].set_ylim((0, 1))

plt.savefig('_Fig1/a_activity.png')
