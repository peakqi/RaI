...
for i in range(VIEW_SIZE_shape):
    view_x_[i],_ = affine_transform1(np.reshape(view_shape[i], [1, 784])) #rtt



disabled_related=14
f, a = plt.subplots(3, VIEW_SIZE_shape)
for i in range(VIEW_SIZE_shape): #left-right
    print('shape:'+str(i))

    a[0][i].clear()
    a[0][i].imshow(np.reshape(view_x_[i], (28, 28)), cmap='gray')
    ...
    view_decoded_data1, we0, we1, we2, we3, wem, wd0, wd1, wd2, wd3, wdd, en0_, en1_, en2_, en3_, ff_encoded_, encoded_1, de0_, de1_, de2_, de3_, ddr_ = sess.run(
        [decoded, weights_en0, weights_en1, weights_en2, weights_en3, weights_mid,
         ...
         encoded, de0, de1,
         de2, de3, decoded],
        {tf_x: np.reshape(view_x_[i],[1,784]),
         ph_encoded: view_ph_encoded_,
         ph_switch: view_ph_switch_,
         ph_dis_e: view_ph_dis_e_})
    a[1][i].imshow(np.reshape(view_decoded_data1, (28, 28)), cmap='gray')

    view_ph_encoded_ = np.zeros(shape=[1, n_encoded])
    view_ph_switch_ = np.ones(shape=[1])
    view_ph_dis_e_ = np.ones(shape=[1, n_encoded])
    view_ph_dis_e_[0,disabled_related]=0
    view_decoded_data1, we0, we1, we2, we3, wem, wd0, wd1, wd2, wd3, wdd, en0_, en1_, en2_, en3_, ff_encoded_, encoded_1, de0_, de1_, de2_, de3_, ddr_ = sess.run(
        [decoded, weights_en0, weights_en1, weights_en2, weights_en3, weights_mid,
         weights_de0, weights_de1, weights_de2, weights_de3, weights_ddr, en0, en1, en2, en3, ff_encoded,
         encoded, de0, de1,
        ...
        {tf_x: np.reshape(view_x_[i],[1,784]),
         ...
         ph_switch: view_ph_switch_,
         ph_dis_e: view_ph_dis_e_})
    a[2][i].imshow(np.reshape(view_decoded_data1, (28, 28)), cmap='gray')
...
plt.savefig('./corr_digit/4disble_RelatedNeuron' +str(disabled_related)+ 'toInvestigateDigit'+str(digit_investigate) + '.png')
