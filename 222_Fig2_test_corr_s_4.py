...
for k in range(9):
    ...
    f1.suptitle(str(k));
    plt.savefig('./corr_s/1layer' + str(k) + '.png')
...
plt.savefig('./corr_s/2whole_layer'  + '.png')



#./corr_s/test_move    significant colorful point move
...
plt.savefig('./corr_s/3test_move'  + '.png')
...
for m in range(0,32,1):
 #(3, 32)
    ...
    plt.savefig('./corr_s/4test_disable_nx_' +str(m) + '.png')
...
for k in range(32):
    ...
    plt.savefig('./corr_s/5test_chg_nx_' + str(k) + '.png')
...
for i in range(BATCH_SIZE):
    # test_x[i, :] = affine_transform_rand_r(np.reshape(test_x[i], [1, 784]))  # rtt
    posi[i] = np.random.random([1]) * 0.2+0.5
    test_x[i] = affine_transform_delta_s(np.reshape(test_x[i], [1, 784]), posi[i])  # mv x=deltaX (deltx,Batch,784)
...
for i in range(BATCH_SIZE):
    # pred_x[i, :] = affine_transform_rand_r(np.reshape(pred_x[i], [1, 784]))  # rtt
    posi2[i] = np.random.random([1]) * 0.2+0.5
    pred_x[i] = affine_transform_delta_s(np.reshape(pred_x[i], [1, 784]), posi2[i])  # mv x=deltaX (deltx,Batch,784)
...
plt.savefig('./corr_s/6_prediction.png')
...
plt.savefig('./corr_s/7_mse.png')

np.save('s_posi2',posi2)
np.save('s_pred',pred)
np.save('s_mse',mse)
np.save('s_acc',acc)
