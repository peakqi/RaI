...
f, a = plt.subplots(11, N_TEST_IMG)
...
rows = ['{}'.format(row) for row in ['O', 'R', 'W1', 'W9', 'a', 'd_a', 'w','|w|', 'd_w', '|d_w|', 'L']]
LearningRate = 0.001
...
for step in range(rangemax * nx, rangemax * (nx + 1)):
    ...
    if step % rangedvd == 0:
        ...
        max2 = np.max(np.abs(wd_absavg), axis=0)
        min1 = np.min(np.abs(wd_avg), axis=0)
        min2 = np.min(np.abs(wd_absavg), axis=0)
        max3 = np.max(np.abs(wd_diff_avg), axis=0)
        max4 = np.max(np.abs(wd_diff_absavg), axis=0)
        min3 = np.min(np.abs(wd_diff_avg), axis=0)
        min4 = np.min(np.abs(wd_diff_absavg), axis=0)
        for i in range(N_TEST_IMG):
            ...
            # action
            a[4][i].clear()
            a[4][i].scatter(np.arange(sz_act_length), act[i], np.ones(sz_act_length) * 0.2,col.reshape([sz_act_length]))
            a[4][i].set_xticks(())
            a[4][i].set_yticks(())
            a[4][i].set_ylim((0, 1))
            ...
            # delta_action
            a[5][i].clear()
            a[5][i].scatter(np.arange(sz_act_length), dact[i], np.ones(sz_act_length) * 0.2,col.reshape([sz_act_length]))
            a[5][i].set_xticks(())
            a[5][i].set_yticks(())
            a[5][i].set_ylim((-1, 1))

            # averaged W
            a[6][i].scatter(np.arange(count), wd_avg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[6][i].set_xticks(())
            a[6][i].set_yticks(())


            # averaged abs W
            a[7][i].scatter(np.arange(count), wd_absavg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[7][i].set_xticks(())
            a[7][i].set_yticks(())

            # averaged W(t)-W(t-1)
            a[8][i].scatter(np.arange(count), wd_diff_avg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[8][i].set_xticks(())
            a[8][i].set_yticks(());  # max1[i]=1;


            # averaged |W(t)-W(t-1)|
            a[9][i].scatter(np.arange(count), wd_diff_absavg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[9][i].set_xticks(())
            a[9][i].set_yticks(());  # max2[i]=1;

            a[10][i].set_xticks(())
            a[10][i].set_yticks(());


            print('step:', step, '|i:',i, '| min1: %.6f' % min1[i],'| max1: %.6f' % max1[i], '|min2: %.6f' % min2[i],'|max2: %.6f' % max2[i],
            '| min3: %.6f' % min3[i], '| max3: %.6f' % max3[i], '|min4: %.6f' % min4[i], '|max4: %.6f' % max4[i])

        a[10][0].scatter(np.arange(count), loss_rec, np.ones(count) * 0.2, np.ones(count))
        a[10][0].set_ylim((0., 0.02))
