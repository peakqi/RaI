...
view_decoded_data, we0_orig, we1, we2, we3, wem, wd0, wd1, wd2, wd3, wdd, en0_, en1_, en2_, en3_, ff_encoded_, de0_, de1_, de2_, de3_, ddr_ = sess.run(
                [decoded, weights_en0, weights_en1, weights_en2, weights_en3, weights_mid,
                 weights_de0, weights_de1, weights_de2, weights_de3, weights_ddr, en0, en1, en2, en3, ff_encoded, de0,
                 de1,
                 de2, de3, decoded],
...
temp=we0_old-we0_orig
for ii in range(16):
    for jj in range(16):
        a[jj][ii].set_xticks(());
        a[jj][ii].set_yticks(());
        W = np.reshape(temp[:, tt], [28, 28])
        W[0][0] = mm;
        W[27][27] = -1 * mm
        W[W > mm] = mm;
        W[W < -1 * mm] = -1 * mm;
        a[jj][ii].imshow(W, cmap='bwr');
        tt = tt + 1
# plt.subplots_adjust(left=0.4,right=1,bottom=0,top=1,wspace=0,hspace=0)

f.suptitle('old-orig')
plt.savefig('_Fig4/old-orig'+str(mm)+'.png')



tt=0
f,a=plt.subplots(16,16)
temp=we0_new-we0_orig
for ii in range(16):
    for jj in range(16):
        a[jj][ii].set_xticks(());
        a[jj][ii].set_yticks(());
...
f.suptitle('new-orig')
plt.savefig('_Fig4/new-orig'+str(mm)+'.png')
...
f.suptitle('orig')
plt.savefig('_Fig4/orig'+str(mm)+'.png')
