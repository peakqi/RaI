...
lnstep_list=[-1,0,34,1000,10000,11000,20000]
...
for kk in range(7):
    if kk==0:
        ...
        if kk<4:
            ...
            saver.restore(sess, '/Users/fengqi/Pycharm_py36/QF/LnNew_Forgetting/Forgeting' + str(lnstep) + '/' + type)
        for ii in range(10):
...
plt.savefig('LnNew_Forgetting/_figcompare-digi.png')
...
for kk in range(7):
    if kk==0:
        ...
        for ii in range(10):
            decoded_im = sess.run(decoded,{tf_x: np.reshape(im[ii,:],[1,784]),ph_encoded: ph_encoded_,ph_switch: ph_switch_,ph_dis_e: ph_dis_e_})
            a[kk][ii].imshow(np.reshape(decoded_im,[28,28]),cmap='gray')
plt.savefig('LnNew_Forgetting/_figcompare-x.png')
