...
test_l = mnist.test.labels[:BATCH_SIZE]
...
for kk in range(9):
    ...
    plt.savefig('./tSNE/'+'tsne'+str(kk)+'.png')


    plt.cla();
    ind_sel=np.arange(start1[kk],start1[kk+1])
    leng=np.asarray(start1[kk+1]-start1[kk])
    for ii in range(plot_only):
        x=X[ii];y=Y[ii];s=test_l[ii];vc=vec[ii,:]
        iinn = np.argsort(vc)
        c = cm.rainbow(int(255 * iinn[-1] / leng));
        plt.text(x, y, iinn[-1], backgroundcolor=c, fontsize=9)
    plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
    plt.savefig('./tSNE/'+'tsne_layer'+str(kk)+'.png')
    ...
    # for x, y, s ,xxx in zip(X[:plot_only], Y[:plot_only], test_l[:plot_only],para[:,0]):
        ...
        plt.text(x, y, ' ', backgroundcolor=c, fontsize=9)
        plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
    plt.savefig('./tSNE/'+'tsne_layer'+str(kk)+'_x'+'.png')
    ...
    plt.savefig('./tSNE/' + 'tsne_layer' + str(kk) + '_y' + '.png')
    ...
    for ii in range(BATCH_SIZE):
        ...
        plt.xlim(X.min(), X.max());
        plt.ylim(Y.min(), Y.max());
    plt.savefig('./tSNE/' + 'tsne_layer' + str(kk) + '_s' + '.png')
    ...
    for ii in range(BATCH_SIZE):
        ...
        plt.xlim(X.min(), X.max());
        plt.ylim(Y.min(), Y.max());
    plt.savefig('./tSNE/' + 'tsne_layer' + str(kk) + '_r' + '.png')
    ...
    for jj in range(10):
        ...
        for ii in range(BATCH_SIZE):
            if test_l[ii] == jj:
                ...
                dx=np.int((xx/temp2)*LEN);dy=np.int((yy/temp2)*LEN);
                im[mid+dy:mid+dy+28,mid+dx:mid+dx+28]=np.reshape(test_x[ii,:], (28, 28))-0.5
                if np.sum(np.sum(np.multiply(IM,im)))==0:
                    IM=IM+im
        scipy.misc.imsave('./tSNE/'+ 'tsne_layer' + str(kk)+'digit'+str(jj)+'tsne.png', IM)
    ...
    for ii in range(BATCH_SIZE):
            ...
            if np.sum(np.sum(np.multiply(IM,im)))==0:
                IM=IM+im
    scipy.misc.imsave('./tSNE/'+ 'tsne_layer' + str(kk)+'all_'+'tsne.png', IM)
