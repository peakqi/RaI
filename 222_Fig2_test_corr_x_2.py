...
#mv x=deltaX
def affine_transform6(b_x,deltaX):
    xx = deltaX  # (-0.5,0.5)
    yy = 0
    sx = 1 # (.5,1.2)
    sy = 1
...
test_x, para = affine_transform3(test_x) #(128, 784) All chg to 0.65 size
for i in range(BATCH_SIZE):
    test_x[i, :], para = affine_transform5(np.reshape(test_x[i], [1, 784])) #rtt
    test_x[i, :], para = affine_transform4(np.reshape(test_x[i], [1, 784])) #mv y
test_x_=np.zeros([x_step,BATCH_SIZE,784])
for i in range(x_step):
    test_x_[i,:,:]=affine_transform6(test_x,x[i]) #mv x=deltaX (deltx,Batch,784)
...
view_x, para = affine_transform3(view_x)
for i in range(VIEW_SIZE):
    view_x[i, :], para = affine_transform5(np.reshape(view_x[i], [1, 784])) #rtt
    view_x[i, :], para = affine_transform4(np.reshape(view_x[i], [1, 784])) #mv y
view_x_=np.zeros([v_step,VIEW_SIZE,784])
for i in range(v_step):
    view_x_[i,:,:]=affine_transform6(view_x,v[i]) #mv x=deltaX (deltx,Batch,784)
...
for k in range(9):
    ...
    f1.suptitle(str(k));
    plt.savefig('./corr_x/1layer' + str(k) + '.png')
...
plt.savefig('./corr_x/2whole_layer'  + '.png')



#./corr_x/test_move    significant colorful point move
...
plt.savefig('./corr_x/3test_move'  + '.png')
...
for m in range(0,32,1):
 #(3, 32)
    ...
    plt.savefig('./corr_x/4test_disable_nx_' +str(m) + '.png')
...
for k in range(32):
    ...
    plt.savefig('./corr_x/5test_chg_nx_' + str(k) + '.png')
...
test_x = mnist.test.images[:BATCH_SIZE]
test_x,_= affine_transform3(test_x) #(128, 784)
for i in range(BATCH_SIZE):
    test_x[i, :],_ = affine_transform5(np.reshape(test_x[i], [1, 784])) #rtt
    test_x[i, :],_ = affine_transform4(np.reshape(test_x[i], [1, 784])) #mv y
    posi[i]=np.random.random([1])*0.4-0.2
    test_x[i] =affine_transform6(np.reshape(test_x[i], [1, 784]),posi[i]) #mv x=deltaX (deltx,Batch,784)
...
pred_x,para = affine_transform3(pred_x) #(128, 784)
for i in range(BATCH_SIZE):
    pred_x[i, :],_ = affine_transform5(np.reshape(pred_x[i], [1, 784])) #rtt
    pred_x[i, :],_ = affine_transform4(np.reshape(pred_x[i], [1, 784])) #mv y
    posi2[i]=np.random.random([1])*0.4-0.2
    pred_x[i]=affine_transform6(np.reshape(pred_x[i], [1, 784]),posi2[i]) #mv x=deltaX (deltx,Batch,784)
...
plt.savefig('./corr_x/6_prediction.png')
...
plt.savefig('./corr_x/7_mse.png')

np.save('x_posi2',posi2)
np.save('x_pred',pred)
np.save('x_mse',mse)
np.save('x_acc',acc)