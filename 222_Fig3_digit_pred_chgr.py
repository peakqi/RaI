...
for i in range(BATCH_SIZE):
    ...
    train_x[i, :],_ = affine_transform_60(np.reshape(train_x[i], [1, 784]))  # rtt
    train_x[i, :] = affine_transform_rand_r(np.reshape(train_x[i], [1, 784]))  # rtt
...
for i in range(BATCH_SIZE):
    ...
    train2_x[i, :], _ = affine_transform_60(np.reshape(train2_x[i], [1, 784]))
    train2_x[i, :]= affine_transform_rand_r(np.reshape(train2_x[i], [1, 784]))  # rtt
