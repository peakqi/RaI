...
for i in range(BATCH_SIZE):
    test_x[i, :]= affine_transform1(np.reshape(test_x[i], [1, 784]))
...
for nn in range(32):
    ...
    print('neuron:'+str(nn))
    xx=9;yy=10
    f, a = plt.subplots(xx, yy)
    n_ind=nn+start1[ll]
    a_vec=act[:,n_ind]
    s_ind=np.argsort(a_vec)[::-1][:4096]
    kk=0
    a[0][0].clear()
    ssum=np.zeros([28,28])
    for i in range(xx):
        for j in range(yy):
            im=np.reshape(test_x[s_ind[kk],:], (28, 28))
            a[i][j].imshow(im, cmap='gray')
            a[i][j].set_xticks(())
            a[i][j].set_yticks(())
            if kk<10:
                ssum=ssum+im
            kk=kk+1
    a[i][j].imshow(np.reshape(ssum, (28, 28)), cmap='gray')
    ssum_fix=ssum
    ssum[ssum <= ssum.max()*0.1] = 0
    a[i][j-1].imshow(np.reshape(ssum, (28, 28)), cmap='gray')
    ssum=ssum_fix
    ssum[ssum <= ssum.max() * 0.3] = 0
    a[i][j-2].imshow(np.reshape(ssum, (28, 28)), cmap='gray')
    ssum = ssum_fix
    ssum[ssum <= ssum.max() * 0.5] = 0
    a[i][j - 3].imshow(np.reshape(ssum, (28, 28)), cmap='gray')
    ssum = ssum_fix
    ssum[ssum <= ssum.max() * 0.7] = 0
    a[i][j - 4].imshow(np.reshape(ssum, (28, 28)), cmap='gray')
    ssum = ssum_fix
    ssum[ssum <= ssum.max() * 0.9] = 0
    a[i][j - 5].imshow(np.reshape(ssum, (28, 28)), cmap='gray')

    aa=test_x[s_ind[0:20], :]
    p=np.ones([784])
    for jj in range(784):
        if aa[:,jj].max()>0:
            [s,p[jj]]=ss.ttest_1samp(aa[:,jj],0)
        else:
            p[jj]=1
    sig=1-p
    a[i][j - 9].clear()
    ...
    a[i][j-9].set_xticks(())
    a[i][j-9].set_yticks(())
    a[i][j - 9].imshow(np.reshape(sig, (28, 28)), cmap='gray')
    plt.savefig('./n_top9/layer' + str(ll) + 'n_' + str(nn) + 'top9.png')
