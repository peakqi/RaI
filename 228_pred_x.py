...
BATCH_SIZE = 1024
xx = np.random.uniform(low=-0.2, high=0.2, size=BATCH_SIZE)
mnist = input_data.read_data_sets('./mnist', one_hot=False)  # use not one-hotted target data
test_x = mnist.test.images[:BATCH_SIZE]
test_x = affine_transform_60(test_x)
for i in range(BATCH_SIZE):
    test_x[i,:]=affine_transform_rand_r(np.reshape(test_x[i,:], [1, 784]))
    test_x[i, :] = affine_transform_rand_y(np.reshape(test_x[i,:], [1, 784]))
    test_x[i, :]= affine_transform_delta_x(np.reshape(test_x[i,:], [1, 784]),xx[i])
...
nx_ind=[8,12,13,15,22,24,26,28,29]
svr = SVR()
svr.fit(ff_encoded_[:,nx_ind], xx)



xx2 = np.random.uniform(low=-0.2, high=0.2, size=BATCH_SIZE)
test_x = mnist.test.images[BATCH_SIZE:BATCH_SIZE*2]
test_x = affine_transform_60(test_x)
for i in range(BATCH_SIZE):
    test_x[i,:]=affine_transform_rand_r(np.reshape(test_x[i,:], [1, 784]))
    test_x[i, :] = affine_transform_rand_y(np.reshape(test_x[i,:], [1, 784]))
    test_x[i, :]= affine_transform_delta_x(np.reshape(test_x[i,:], [1, 784]),xx2[i])
...
plt.cla()
prd=svr.predict(ff_encoded_[:,nx_ind])
plt.scatter(xx2,prd,xx2*0+1,xx2*0+1)
[cc,vv]=np.corrcoef(xx2,prd)
plt.xlabel('Real x')
plt.ylabel('predicted x')

[qq,pp]=scipy.stats.pearsonr(xx2,prd)
plt.title(str(qq)+'-'+str(pp))
from pylab import *
m,b = polyfit(xx2, prd, 1)
plot(xx2, prd, 'yo', xx2, m*xx2+b, '--k')

plt.savefig('./prediction/'+'pred_x.png')
