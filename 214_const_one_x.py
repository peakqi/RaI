...
def affine_transform2(b_x):
    xx = np.random.uniform(low=-0.5, high=-0.1, size=1)  # (-0.5,0.5)
...
test_x = mnist.test.images[:BATCH_SIZE]*0
test_x_=test_x*0;
jj=0
for i in range(10000):
    if 1==mnist.test.labels[i]:
        test_x[jj,:] = mnist.test.images[i]
        test_x_[jj,:], para = affine_transform2(np.reshape(test_x[jj,:],[1,784]))
        jj = jj + 1
