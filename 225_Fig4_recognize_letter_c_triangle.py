...
test_x,text_l = mnist.test.next_batch(BATCH_SIZE)



############Square
for i in range(BATCH_SIZE):
    im = square(np.reshape(test_x[i], [1, 784]))
    im=affine_transform_60(im)
    test_x[i, :]= affine_transform1(np.reshape(im, [1, 784]))
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/square' + '.png')



############triangle
mask1=np.ones([14,28])
mask2=np.zeros([14,28])
mask_up=np.concatenate((mask1,mask2),axis=0)

for i in range(BATCH_SIZE):
    squares = square(np.zeros([1, 784]))
    squares = affine_transform_60(squares)
    im=affine_transform_delta_r(squares, 0.25)
    im = affine_transform_delta_sx(im, 0.66)
    imp=np.reshape(im,[28,28])
    imp=np.multiply(imp,mask_up)
    test_x[i, :]= affine_transform_rand_noScale(np.reshape(imp, [1, 784]))
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/triangle' + '.png')
...
for i in range(BATCH_SIZE):
    squares = square(np.zeros([1, 784]))
    squares = affine_transform_60(squares)
    im1=affine_transform_delta_s(squares, 0.5)
    im2 = affine_transform_delta_s(squares, 0.6)
    im=im2-im1
    test_x[i, :]= affine_transform1(np.reshape(im, [1, 784]))
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/square_ring' + '.png')



############rand structures
text_x=np.zeros([BATCH_SIZE,784])
mnist_x,mnist_l = mnist.test.next_batch(BATCH_SIZE)
mask1=np.ones([14,28])
mask2=np.zeros([14,28])
mask_up=np.concatenate((mask1,mask2),axis=0)
mask3=np.ones([28,14])
mask4=np.zeros([28,14])
mask_left=np.concatenate((mask3,mask4),axis=1)
for i in range(BATCH_SIZE):
    im=np.multiply(mnist_x[i,:],np.reshape(mask_left, [1, 784]))
    test_x[i, :]= affine_transform_rand_noScale(np.reshape(im, [1, 784]))
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/rand_struct' + '.png')


# mirro
test_x,text_l = mnist.test.next_batch(BATCH_SIZE)
for i in range(10):
    im=np.reshape(test_x[i], [28, 28])
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/flip_x' + '.png')


############2 digit
VIEW_SIZE=100
mnist_x = mnist.test.images[:VIEW_SIZE]

for i in range(BATCH_SIZE):
    im1=affine_transform_delta_sx(np.reshape(mnist_x[i], [1, 784]),0.5)
    ima=im1.reshape([28,28])
    im2 = affine_transform_delta_sx(np.reshape(mnist_x[i+10], [1, 784]),0.5)
    imb = im2.reshape([28, 28])
    im=np.concatenate((ima[:,7:21],imb[:,7:21]),axis=1)
    test_x[i, :]= affine_transform1(np.reshape(im, [1, 784]))
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/2digit' + '.png')
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/x' + '.png')
...
mask2=np.zeros([14,28])
mask_up=np.concatenate((mask1,mask2),axis=0)
mask_up_flat=np.reshape(mask_up,[1,784])
for i in range(BATCH_SIZE):
    squares = square(np.zeros([1, 784]))
    x_stick=affine_transform_delta_sx(squares,0.1)
    x_up=np.multiply(x_stick, mask_up_flat)
    y_left = affine_transform_delta_r(x_up, 0.5)
    y_left_s = affine_transform_rand_sx_full(y_left)
    y_left_s1 = affine_transform_rand_r(y_left_s)

    y_left_s = affine_transform_rand_sx_full(y_left)
    y_left_s2 = affine_transform_rand_r(y_left_s)
    test_x[i,:]=affine_transform_rand_noScale(y_left_s1+y_left_s2)
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/v' + '.png')
