...
def cal_w_stats(we, wep):
    ...
    return np.concatenate((wed_max.reshape([1]), wed_min.reshape([1]), wed_avg.reshape([1]), wed_abs_avg.reshape([1]),
                           we_std.reshape([1]), we_avg.reshape([1])))
...
f, a = plt.subplots(10, N_TEST_IMG)
...
rows = ['{}'.format(row) for row in ['O', 'R', 'W1', 'W9', 'a', 'd_a', 'w', 'd_w', '|d_w|', 'L']]
LearningRate = 0.001
ph_lr_ = np.ones(shape=[]) * LearningRate
rangemax = 5000000;
rangedvd = 2000;rangedvd100 = 200000
count = 0
nx = -1
nx = 1 + nx
...
for step in range(rangemax * nx, rangemax * (nx + 1)):
    ...
    if step % rangedvd == 0:
        ...
        for i in range(N_TEST_IMG):
            ...
            a[4][i].clear()
            a[4][i].scatter(np.arange(sz_act_length), act[i], np.ones(sz_act_length) * 0.2,
                            col.reshape([sz_act_length]))
            a[4][i].set_xticks(())
            a[4][i].set_yticks(())
            a[4][i].set_ylim((0, 1))

            a[5][i].clear()
            a[5][i].scatter(np.arange(sz_act_length), dact[i], np.ones(sz_act_length) * 0.2,
                            col.reshape([sz_act_length]))
            a[5][i].set_xticks(())
            a[5][i].set_yticks(())
            a[5][i].set_ylim((-1, 1))

            max1 = np.max(np.abs(wd_diff_avg), axis=0)
            max2 = np.max(np.abs(wd_diff_absavg), axis=0)
            a[6][i].scatter(np.arange(count), wd_avg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[6][i].set_xticks(())
            a[6][i].set_yticks(())

            a[7][i].scatter(np.arange(count), wd_diff_avg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[7][i].set_xticks(())
            a[7][i].set_yticks(());  # max1[i]=1;
            # a[6][i].set_title(str('%.2f' % np.round(max1[i], decimals=4)));
            # a[6][i].set_ylim((-max1[i], max1[i]))

            a[8][i].scatter(np.arange(count), wd_diff_absavg[:, i], np.ones(count) * 0.2, np.ones(count))
            a[8][i].set_xticks(())
            a[8][i].set_yticks(());  # max2[i]=1;

            a[9][i].set_xticks(())
            a[9][i].set_yticks(());

        a[9][0].scatter(np.arange(count), loss_rec, np.ones(count) * 0.2, np.ones(count))
        a[9][0].set_ylim((0., 0.02))
