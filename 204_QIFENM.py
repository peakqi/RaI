...
for step in range(50):
    ...
    encoded_, en3_ = sess.run([ encoded, en2], {tf_x: b_x,ph_encoded:ph_encoded_,ph_switch:ph_switch_})
    if step==0:
        e_col=encoded_
        n_col=en3_[:,nn]
    else:
...
aa.clear()
aa.imshow(np.reshape(decoded_, (28, 28)), cmap='gray')
plt.draw();
plt.pause(0.01)
a=1
plt.ioff()
...
for nn in range(num):
    for pp in range(num):
        ...
        for step in range(50):
            b_x, b_y = mnist.train.next_batch(BATCH_SIZE)
            encoded_, en3_ = sess.run([ encoded, en0], {tf_x: b_x,ph_encoded:ph_encoded_,ph_switch:ph_switch_})
            if step==0:
                e_col=encoded_
                n_col=en3_[:,nn]
            else:
...
for nn in range(num):
    for pp in range(num):
        ...
        for step in range(50):
            b_x, b_y = mnist.train.next_batch(BATCH_SIZE)
            encoded_, en3_ = sess.run([ encoded, en1], {tf_x: b_x,ph_encoded:ph_encoded_,ph_switch:ph_switch_})
            if step==0:
                e_col=encoded_
                n_col=en3_[:,nn]
            else:
