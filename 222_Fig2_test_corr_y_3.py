...
for k in range(9):
    ...
    f1.suptitle(str(k));
    plt.savefig('./corr_y/1layer' + str(k) + '.png')
...
plt.savefig('./corr_y/2whole_layer'  + '.png')



#./corr_y/test_move    significant colorful point move
...
plt.savefig('./corr_y/3test_move'  + '.png')
...
for m in range(0,32,1):
 #(3, 32)
    ...
    plt.savefig('./corr_y/4test_disable_nx_' +str(m) + '.png')
...
for k in range(32):
    ...
    plt.savefig('./corr_y/5test_chg_nx_' + str(k) + '.png')
...
for i in range(BATCH_SIZE):
    ...
    test_x[i] = affine_transform_delta_y(np.reshape(test_x[i], [1, 784]), posi[i])  # mv x=deltaX (deltx,Batch,784)
...
for i in range(BATCH_SIZE):
    ...
    pred_x[i, :] = affine_transform_rand_x(np.reshape(pred_x[i], [1, 784]))  # mv y
    posi2[i] = np.random.random([1]) * 0.4 - 0.2
    pred_x[i] = affine_transform_delta_y(np.reshape(pred_x[i], [1, 784]), posi2[i])  # mv x=deltaX (deltx,Batch,784)
...
plt.savefig('./corr_y/6_prediction.png')
...
plt.savefig('./corr_y/7_mse.png')

np.save('y_posi2',posi2)
np.save('y_pred',pred)
np.save('y_mse',mse)
np.save('y_acc',acc)