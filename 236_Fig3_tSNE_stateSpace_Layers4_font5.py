...
def plot_with_labels(lowDWeights, labels):
    ...
    for x, y, s in zip(X, Y, labels):
        c = plt.rainbow(int(255 * s / 9)); plt.text(x, y, s, backgroundcolor=c, fontsize=5)
...
for x, y, s,iind in zip(X[:plot_only], Y[:plot_only], test_l[:plot_only],iii):
    ...
    plt.text(x, y, s, backgroundcolor=c, fontsize=5 )
plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/'+'font5_tsne'+str(kk)+'.png')
...
for jj in range(4):
    ...
    for ii in range(plot_only):
        ...
        plt.text(x, y, iinn[32-jj-1], backgroundcolor=c, fontsize=5 )
    plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
    plt.savefig('./tSNE/'+'font5_tsne_importNeuron_'+str(jj)+'.png')
...
# for x, y, s ,xxx in zip(X[:plot_only], Y[:plot_only], test_l[:plot_only],para[:,0]):
    ...
    plt.text(x, y, ' ', backgroundcolor=c, fontsize=5 )
    plt.xlim(X.min(), X.max());plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/'+'font5_tsne_layer'+str(kk)+'_x'+'.png')
...
plt.savefig('./tSNE/' + 'font5_tsne_layer' + str(kk) + '_y' + '.png')
...
for ii in range(BATCH_SIZE):
    ...
    plt.xlim(X.min(), X.max());
    plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/' + 'font5_tsne_layer' + str(kk) + '_s' + '.png')
...
for ii in range(BATCH_SIZE):
    ...
    plt.xlim(X.min(), X.max());
    plt.ylim(Y.min(), Y.max());
plt.savefig('./tSNE/' + 'font5_tsne_layer' + str(kk) + '_r' + '.png')
    ...
    for ii in range(BATCH_SIZE):
        if test_l[ii] == jj:
            ...
            if np.sum(np.sum(np.multiply(IM,im)))==0:
                IM=IM+im
    scipy.misc.imsave('./tSNE/'+ 'font5_tsne_layer' + str(kk)+'digit'+str(jj)+'tsne.png', IM)
...
for ii in range(BATCH_SIZE):
        ...
        if np.sum(np.sum(np.multiply(IM,im)))==0:
            IM=IM+im
scipy.misc.imsave('./tSNE/'+ 'font5_tsne_layer' + str(kk)+'all_'+'font5_tsne.png', IM)
