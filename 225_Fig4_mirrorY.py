...
test_xx,text_l = mnist.test.next_batch(1000)
jj=0
for i in range(1000):
    if jj==text_l[i]:
        im=np.reshape(test_xx[i], [28, 28])
        im=np.flip(im,axis=0)
        test_x[jj, :]= affine_transform1(np.reshape(im, [1, 784]))
        jj=jj+1
        if jj==10:
            break
...
for i in range(10):
    ...
    a[0][i].set_yticks(());
    a[1][i].set_yticks(());
plt.savefig('./Strange_Recognition/mirror_y' + '.png')
